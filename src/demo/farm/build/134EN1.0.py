...
CONFIG = {
    ...
    "MODE_NAME": "team",           # team=练级, resource134=资源打捞
...
def check_win_equip_drop(resp_data):
    ...
    return collected
"""
def extract_fairy_exp_gain_from_resp(resp_data):
    # 简化实现，返回0（避免复杂逻辑）
    return 0


def apply_fairy_exp_gain_from_resp(resp_data):
    return 0

def check_battle_exp(resp_data, spot_id):
    any_zero = False
    all_zero = False
    ...
    if gun_exp_list:
        ...
        for item in gun_exp_list:
            ...
            exp_int = int(exp_val) if exp_val.isdigit() else 0
            exp_details.append(f"{gun_uid[-4:]}: +{exp_val}")
            if gun_uid in gun_map and exp_int > 0:
                gun_map[gun_uid]["runtime_gained_exp"] = gun_map[gun_uid].get("runtime_gained_exp",0) + exp_int
                current_cfg.setdefault("maxed_member_uids", set()).discard(gun_uid)
            is_zero = (exp_val == "0")
            zero_flags.append(is_zero)
            if is_zero:
                current_cfg.setdefault("maxed_member_uids", set()).add(gun_uid)
                any_zero = True
...
def print_run_confirm_menu():
    ...
    if is_13_4_resource_farm_stage():
        ...
        print("模式：练级五人模式" if CONFIG.get("MODE_NAME") == "team" else "打捞单人模式")
    print(f"满级停机：{'开启' if CONFIG.get('STOP_ON_MAX_LEVEL') else '关闭'}")
    print("----------------------------------")
...
def get_team_runtime_seconds(team_cfg):
    total = team_cfg.get("runtime_seconds",0)
    if TEAM_PROGRESS_STATE.get("current_active_team_id") == team_cfg.get("team_id"):
        started = TEAM_PROGRESS_STATE.get("current_active_started_at")
        if started:
...
def get_team_member_progress(team_cfg):
    total_exp = 0
    maxed = team_cfg.get("maxed_member_uids", set())
    for g in team_cfg.get("guns", []):
        if str(g.get("id")) in maxed:
            continue
        total_exp += g.get("runtime_gained_exp", 0)
    return total_exp, 1, total_exp, 0  # 简化
...
def farm_mission_epa(client, team_id, mvp_gen):
    ...
    for step, next_spot in enumerate(route, 1):
        ...
        if CONFIG.get("MODE_NAME") == "team" and all_zero:
            TEAM_SWITCH_PENDING = True
            mark_current_training_team_completed()
            print("    [*] 当前编队成员 EXP 均为 0，该梯队已完成。")
        elif any_zero and CONFIG.get("STOP_ON_MAX_LEVEL"):
            #stop_macro_flag = True
            #stop_micro_flag = True
            print("    [*] 检测到满级，已触发自动停机。")
        """
